...
class UrlKeywordExtractorTests < Test::Unit::TestCase
  ...
  EXPECTED = {keywords:%w(what is the weather like on mars)}
  ...
  def test_extractor_returns_hash
    keywords = @extractor.extract SIMPLE_TEST_URL
    assert keywords.kind_of?(Hash)
  end

  def test_correct_for_simple_test
    result = @extractor.extract(SIMPLE_TEST_URL)
    assert_equal(EXPECTED[:keywords], result[:keywords])
  end

  def test_correct_for_case_only_test
    result = @extractor.extract(CASE_ONLY_TEST_URL)
    assert_equal(EXPECTED[:keywords], result[:keywords])
  end

  def test_correct_for_difficult_test
    result = @extractor.extract(DIFFICULT_TEST_URL)
    assert_equal(EXPECTED[:keywords], result[:keywords])
  end

  def test_performant_for_simple_test
    time = Benchmark.measure {@extractor.extract(SIMPLE_TEST_URL)}
    assert time.real < 0.01
  end

  def test_performant_for_case_only_test
    time = Benchmark.measure {@extractor.extract(CASE_ONLY_TEST_URL)}
    assert time.real < 0.01
  end

  def test_performant_for_difficult_test
    time = Benchmark.measure {@extractor.extract(DIFFICULT_TEST_URL)}
    assert time.real < 0.01
  end
